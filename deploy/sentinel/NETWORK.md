...
Stage 32 makes path selection independent of relationship discovery order. Shortest paths win; confirmed paths take precedence when hop counts tie. A longer confirmed route does not replace a shorter inferred route. Loops and duplicate links do not inflate asset counts, and missing assets are not traversed. Multiple graph paths do not establish operational failover.
