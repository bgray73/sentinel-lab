...
import type { RecoveryDrillService } from './recovery/drills.js';
import type { GuestRecoveryDrillService } from './recovery/guest-drills.js';
import type { RecoveryReadinessService } from './recovery/readiness.js';
import type { RecoveryAlertService } from './recovery/alerts.js';
import type { CapacityForecastService } from './capacity/forecast.js';
import type { CapacityAlertService } from './capacity/alerts.js';
import type { NetworkService } from './network/service.js';
import type { NetworkAlertService } from './network/alerts.js';

export function createApp(store: Store, monitoring?: MonitoringService, telemetry?: TelemetryService, cmdb?: CmdbService, logs?: LokiService, hardware?: HardwareService, log: StructuredLogger = defaultLogger, auth: AuthConfig = authConfigFromEnvironment(), security: SecurityAuditService = new SecurityAuditService(), backups?: BackupService, collectors?: CollectorService, serviceNow?: ServiceNowCmdbService, proxmoxOperations?: ProxmoxOperationsService, pbsHealth?: PbsHealthService, recoveryDrills?: RecoveryDrillService, guestDrills?: GuestRecoveryDrillService, recoveryReadiness?: RecoveryReadinessService, recoveryAlerts?: RecoveryAlertService, capacityForecasts?: CapacityForecastService, capacityAlerts?: CapacityAlertService, network?: NetworkService, networkAlerts?: NetworkAlertService) {
  const app = express();
  ...
  app.get('/metrics',async(_req,res)=>{
    ...
    await monitoring.ready;if(telemetry)await telemetry.ready;if(hardware)await hardware.ready;if(network)await network.ready;res.set('Content-Type','text/plain; version=0.0.4; charset=utf-8');return res.send(`${monitoring.prometheus()}${telemetry?.prometheus()||''}${hardware?.prometheus()||''}${await security.prometheus()}${backups?await backups.prometheus():''}${collectors?await collectors.prometheus():''}${serviceNow?await serviceNow.prometheus():''}${proxmoxOperations?await proxmoxOperations.prometheus():''}${pbsHealth?await pbsHealth.prometheus():''}${recoveryDrills?await recoveryDrills.prometheus():''}${guestDrills?await guestDrills.prometheus():''}${recoveryReadiness?await recoveryReadiness.prometheus():''}${recoveryAlerts?await recoveryAlerts.prometheus():''}${capacityForecasts?await capacityForecasts.prometheus():''}${capacityAlerts?await capacityAlerts.prometheus():''}${network?.prometheus()||''}${networkAlerts?await networkAlerts.prometheus():''}`);
  });
  ...
  app.get('/api/recovery/drills',async(_req,res)=>{if(!recoveryDrills)return res.status(503).json({error:'Recovery drill service is not available'});await recoveryDrills.ready;return res.json(recoveryDrills.snapshot())});
  app.post('/api/recovery/drills',async(_req,res)=>{if(!recoveryDrills)return res.status(503).json({error:'Recovery drill service is not available'});try{await recoveryDrills.ready;return res.json(await recoveryDrills.run('manual'))}catch(error){return res.status(502).json({error:error instanceof Error?error.message:'Recovery drill failed'})}});
  app.get('/api/recovery/guest-drills',async(_req,res)=>{if(!guestDrills)return res.status(503).json({error:'Guest recovery drill service is not available'});await guestDrills.ready;return res.json(guestDrills.snapshot())});
  app.post('/api/recovery/guest-drills',async(req,res)=>{if(!guestDrills)return res.status(503).json({error:'Guest recovery drill service is not available'});try{await guestDrills.ready;return res.json(await guestDrills.run(String(req.body?.confirmation||'')))}catch(error){return res.status(400).json({error:error instanceof Error?error.message:'Guest recovery drill failed'})}});
  app.get('/api/recovery/readiness',async(_req,res)=>{if(!recoveryReadiness)return res.status(503).json({error:'Recovery readiness service is not available'});return res.json(await recoveryReadiness.snapshot())});
  app.get('/api/recovery/alerts',async(_req,res)=>{if(!recoveryAlerts)return res.status(503).json({error:'Recovery alert service is not available'});await recoveryAlerts.ready;return res.json(recoveryAlerts.snapshot())});
  app.post('/api/recovery/alerts/evaluate',async(_req,res)=>{if(!recoveryAlerts)return res.status(503).json({error:'Recovery alert service is not available'});try{await recoveryAlerts.ready;return res.json(await recoveryAlerts.evaluate('manual'))}catch(error){return res.status(400).json({error:error instanceof Error?error.message:'Recovery alert evaluation failed'})}});
  ...
  app.get('/api/capacity/forecasts',async(_req,res)=>{if(!capacityForecasts)return res.status(503).json({error:'Capacity forecast service is not available'});return res.json(await capacityForecasts.snapshot())});
  app.get('/api/capacity/alerts',async(_req,res)=>{if(!capacityAlerts)return res.status(503).json({error:'Capacity alert service is not available'});await capacityAlerts.ready;return res.json(capacityAlerts.snapshot())});
  app.post('/api/capacity/alerts/evaluate',async(_req,res)=>{if(!capacityAlerts)return res.status(503).json({error:'Capacity alert service is not available'});try{await capacityAlerts.ready;return res.json(await capacityAlerts.evaluate('manual'))}catch(error){return res.status(400).json({error:error instanceof Error?error.message:'Capacity alert evaluation failed'})}});
  app.get('/api/network/interfaces',async(_req,res)=>{if(!network)return res.status(503).json({error:'Network service is not available'});await network.ready;return res.json(network.snapshot())});
  app.post('/api/network/interfaces/collect',async(_req,res)=>{if(!network)return res.status(503).json({error:'Network service is not available'});try{await network.ready;return res.json(await network.collect())}catch(error){return res.status(502).json({error:error instanceof Error?error.message:'Network collection failed'})}});
  app.get('/api/network/alerts',async(_req,res)=>{if(!networkAlerts)return res.status(503).json({error:'Network alert service is not available'});await networkAlerts.ready;return res.json(networkAlerts.snapshot())});
  app.post('/api/network/alerts/evaluate',async(_req,res)=>{if(!networkAlerts)return res.status(503).json({error:'Network alert service is not available'});try{await networkAlerts.ready;return res.json(await networkAlerts.evaluate('manual'))}catch(error){return res.status(400).json({error:error instanceof Error?error.message:'Network alert evaluation failed'})}});
