...
export type TopologyNode = { id:string; type:TopologyNodeType; name:string; state:string; health:HealthStatus; source:'proxmox'|'docker'|'monitoring'|'hardware'|'network'; detail?:string };
...
export type CorrelationGroup = { id:string; rootNodeId:string; title:string; explanation:string; confidence:number; severity:AlertSeverity; incidentIds:string[]; affectedServices:string[]; affectedAssets:TopologyImpactAsset[]; impact:TopologyImpactSummary; evidence:string[] };
...
export type CiClass='node'|'vm'|'lxc'|'storage'|'docker_host'|'application'|'container'|'service'|'database'|'network'|'network_interface'|'physical_server'|'switch'|'router'|'ups'|'pdu'|'storage_appliance'|'other';
export type CiLifecycle='active'|'stale'|'retired';
export type CiCriticality='low'|'medium'|'high'|'critical';
export type ConfigurationItem={id:string;externalId:string;class:CiClass;name:string;source:'proxmox'|'docker'|'monitoring'|'hardware'|'network'|'manual';lifecycle:CiLifecycle;status:string;environment:string;owner:string;criticality:CiCriticality;tags:string[];attributes:Record<string,string|number|boolean|null>;firstSeenAt:string;lastSeenAt:string;updatedAt:string;version:number};
